...
for i = 1:m
	tmp_y(y(i)) = 1;
	tmp_x = a3(i, :);
	J = J - (log(tmp_x) * tmp_y + log(1 - tmp_x) * (1 - tmp_y));
	tmp_y(y(i)) = 0;
end

J = J / m;

reg = nn_params' * nn_params - Theta1(:, 1)' * Theta1(:, 1) - Theta2(:, 1)' * Theta2(:, 1);
J = J + reg * lambda / m / 2;
...
for i = 1:m
	tmp_y(y(i)) = 1;
	delta3 = a3(i, :)' - tmp_y;
	delta2 = Theta2'(2:size(Theta2, 2), :) * delta3 .* sigmoidGradient(z2(i, :)');
	d2 = d2 + delta3 * a2(i, :);
	d1 = d1 + delta2 * a1(i, :);
	tmp_y(y(i)) = 0;
end
